...
GOOS=linux go build -tags=jsoniter -v -ldflags="-s -w -X main.VersionStr=$version" -o kdataset ./cmd/kdataset
cp kdataset kdataset-linux
tar -cvzf kdataset-linux-amd64-$version.tar.gz kdataset README.md

GOOS=darwin go build -tags=jsoniter -v -ldflags="-s -w -X main.VersionStr=$version" -o kdataset ./cmd/kdataset
cp kdataset kdataset-osx
tar -cvzf kdataset-darwin-amd64-$version.tar.gz kdataset README.md

GOOS=windows go build -tags=jsoniter -v -ldflags="-s -w -X main.VersionStr=$version" -o kdataset.exe ./cmd/kdataset
